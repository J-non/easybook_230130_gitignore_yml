...
public class AladdinApiBO {
	...
	public Book detailBook(int itemId) {
		...
		try {
			URL itemListUrl = new URL("http://www.aladin.co.kr/ttb/api/ItemLookUp.aspx?ttbkey=" + TTBKEY
					+ "&itemIdType=ItemId&ItemId="+ itemId + "&Cover=Big&output=JS&Version=20131101");
			BufferedReader bf;
			bf = new BufferedReader(new InputStreamReader(itemListUrl.openStream(), "UTF-8"));
			result = bf.readLine();
			...
			for(int i = 0; i < items.size(); i++) {
				...
				book.setSalesPoint(Integer.parseInt(item.get("salesPoint") + ""));
			}
